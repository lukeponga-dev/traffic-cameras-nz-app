...
export function CameraCard({ camera }: { camera: Camera }) {
    ...
    return (
        <Card className="overflow-hidden transition-all duration-300 bg-card/50 hover:bg-card hover:shadow-lg hover:border-primary/20">
            <Link href={`/cameras/${camera.id}`} className="block group">
                ...
                <CardContent className="p-4 pt-0 flex items-center justify-between text-xs text-muted-foreground">
